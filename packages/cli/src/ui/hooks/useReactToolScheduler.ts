...
export function useReactToolScheduler(
  ...
  const scheduler = useMemo(
    () => {
      // Ensure tool registry is properly initialized for multi-model support
      const toolRegistryPromise = (async () => {
        try {
          let registry = await config.getToolRegistry();
          if (!registry) {
            // Create tool registry if it doesn't exist
            registry = await config.createToolRegistry();
          }
          return registry;
        } catch (error) {
          console.error('Failed to initialize tool registry for scheduler:', error);
          throw error;
        }
      })();
...
export function mapToDisplay(
  ...
  const toolDisplays = toolCalls.map(
    (trackedCall): IndividualToolCallDisplay => {
      ...
      // Calculate duration based on tool call status
      let duration: string | undefined;
      if ('durationMs' in trackedCall && trackedCall.durationMs) {
        // For completed tool calls, use the stored duration
        duration = formatDuration(trackedCall.durationMs);
      } else if ('startTime' in trackedCall && trackedCall.startTime) {
        // For active tool calls, calculate duration from start time
        const durationMs = Date.now() - trackedCall.startTime;
      ...
      > = {
        ...
        startTime: 'startTime' in trackedCall ? trackedCall.startTime : undefined,
        duration,
      };
